...
export interface UseAnalyzeStreamReturn {
  ...
  /**
   * POST /intent for TEXT-driven analytics (Intent 07 preset, no audio):
   * sends `text_query` instead of audio. AI skips STT and runs the same
   * analyze flow. Used by home "Xem phân tích" → /intent-07?preset=...
   */
...
export function useAnalyzeStream(): UseAnalyzeStreamReturn {
  ...
  const submitText = useCallback(
    async (text: string) => {
      ...
      const res = await fetch('/api/v1/intent', {
        ...
        body: JSON.stringify({
          modality: 'voice',
          hint: 'analyze', // dispatch discriminator → analyzing graph
          text_query: text, // AI skips STT and uses this as the question
        }),
      });
  ...
  return { state, submitUtterance, submitText, dispatch };
}
