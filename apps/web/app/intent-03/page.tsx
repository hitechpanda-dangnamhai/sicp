 ...
 * - W3: 5 search.* behavior events WIRED T06 Phiên Sx04-12 (tracking-hooks.ts); cart.item_added baseline T05
...
import {
  trackSearchSuggestedChipTapped,
  trackSearchTypoCorrected,
  trackSearchVariantDegraded,
} from '@/src/features/search/tracking-hooks';
...
export default function Intent03Page() {
  ...
  const handleSuggestedChipTap = useCallback(
    (query: string, position: number) => {
      // D-S04-07 LAW: emit search.suggested_chip_tapped (T06 Phiên Sx04-12).
      trackSearchSuggestedChipTapped({
        query,
        chip_label: query,
  ...
  const handleTypoAccept = useCallback(() => {
    ...
    if (typoCtx) {
      ...
      trackSearchTypoCorrected({
        request_id: stream.state.requestId,
        original_query: typoCtx.original,
        corrected_query: typoCtx.corrected,
        confidence: typoCtx.confidence,
        user_choice: 'accept',
        attempt_n: stream.state.attemptN,
      });
    }
    void postAction(stream.state.requestId, {
      choice: 'accept',
  ...
  const handleRetryAi = useCallback(() => {
    if (!stream.state.requestId) return;
    const degradeCtx = stream.state.variantDegraded;
    if (degradeCtx) {
      // D-S04-03 + D-S04-13 LAW: emit search.variant_degraded user_choice='retry_ai' (T06 Phiên Sx04-12).
      // C-S04-DRIFT-1: state field is `variantDegraded` (NOT `variantDegradedPayload` — handoff drift).
      trackSearchVariantDegraded({
        request_id: stream.state.requestId,
        from: 'ai_augmented',
        to: 'basic_fallback',
        reason: degradeCtx.reason,
        error_code: degradeCtx.error_code,
        trace_id: degradeCtx.trace_id,
        user_choice: 'retry_ai',
        attempt_n: stream.state.attemptN + 1,
      });
    }
  ...
  const handleContinueBasic = useCallback(() => {
    ...
    const degradeCtx = stream.state.variantDegraded;
    if (degradeCtx) {
      // D-S04-03 + D-S04-13 LAW: emit search.variant_degraded user_choice='continue_basic' (T06 Phiên Sx04-12).
      // C-S04-DRIFT-1: state field is `variantDegraded` (NOT `variantDegradedPayload`).
      trackSearchVariantDegraded({
        request_id: stream.state.requestId,
        from: 'ai_augmented',
        to: 'basic_fallback',
