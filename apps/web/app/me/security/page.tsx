...
/**
 * apps/web/app/me/security/page.tsx
 *
 * Stub route — Bảo mật (Security) section.
 *
 * Slice:    S-03 T05 — 3 stub settings routes per AC-37 + DM-10(d)
 *
 * Pattern identical to `apps/web/app/me/notifications/page.tsx` — only
 * differences:
 *   - `section: 'security'` enum value in tracker event
 *   - Title "Bảo mật"
 *   - Icon `shield-check`
 *   - Placeholder copy mentions password + 2FA
 *
 * Per BRIEF non-goal "❌ 2FA implementation (mockup state-F Shield row = stub
 * link only)" — this route never gets real 2FA UI within S-03 scope.
 *
...
export default function MeSecurityStubPage() {
  const router = useRouter();

  React.useEffect(() => {
    try {
      getTracker().track('nav.settings_section_opened', { section: 'security' });
    } catch (err) {
      ...
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 text-center">
          ...
          <div className="text-[12px] text-[#9F1239] leading-[1.5] max-w-[260px]">
            Em sẽ sớm cập nhật trang Bảo mật để anh đổi mật khẩu và bật xác thực 2 lớp tại đây.
          </div>
        </div>
      </div>
    </div>
