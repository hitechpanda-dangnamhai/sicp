...
export interface PostIntentBody {
  ...
  hint?:
    ...
    // S-10 NEW (Intent 07 voice analytics): forwarded to AI, dispatched to
    // analyzing_by_voices when modality=='voice'. Mirrors intent-request.dto.ts.
    | 'analyze';
  /** S-04 NEW per D-S04-03 LAW Adaptive Single Endpoint. */
  mode?: 'ai_augmented' | 'basic_fallback';
  /**
   * S-10 NEW (Intent 07 text-entry): preset question text driving the analyze
   * flow without audio. Forwarded to AI; analyzing_by_voices uses it directly.
   */
  text_query?: string;
  /**
...
export interface PostIntentResumeBody {
  ...
  choice:
    ...
    | 'resolve_remove'
    | 'resolve_replace'
    // S-08 T01 NEW per D-S08-NN-B LAW + C-S08-O resolution (Phiên Sx08-D —
    // manual sync of intent-action.dto.ts choice enum for the Intent 02
    // voice buy clarify path; tech debt recorded as C-S08-O recommending
    // future refactor to z.infer<typeof IntentActionSchema>):
    | 'clarify_pick';
