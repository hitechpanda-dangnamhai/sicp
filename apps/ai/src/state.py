...
class IcpState(TypedDict, total=False):
    ...
    # --- S-09 NEW Phiên Sx09-C per D-S09-NN-A LAW + D-S09-NN-B LAW + D-S09-NN-C LAW ---
    # Intent 04 image recommendation flow internal transient fields (Pattern A reuse).
    # Underscore-prefixed per Phiên Sx04-7 declare-or-drop pattern (NOT surfaced to SSE).
    # Populated by recommend_by_images.py 5-node graph in order:
    #   _detected      ← Node 1 (vision_analyze MCP call result)
    #   _query_desc    ← Node 2 (build_query_desc — composite template for Vespa CLIP)
    #   _visual_hits   ← Node 3 parallel (vespa.image_nearest_neighbor top 30)
    #   _co_purchased  ← Node 3 parallel (analytics.co_purchased category top 20)
    #   _blended       ← Node 4 (composite + sub_scores + match_type attached)
    # Final products event emitted from _blended after Node 5 attach_reasons.
    _detected: dict           # vision.analyze result {category, attributes, ocr_text, confidence, ...}
    _query_desc: str          # C-S09-AK Phiên Sx09-C: composite text from Node 2 → Node 3 MCP visual fetch
    _visual_hits: list        # MCP image_nearest_neighbor top 30 with sub-scores
    _co_purchased: list       # MCP analytics.co_purchased top 20 with freq
    _blended: list            # blend_and_rank node output top 30 with composite + match_type

    # --- S-08 T01.E NEW Phiên Sx08-D per D-S08-NN-A LAW (Voice Session Memory
    #     Hybrid cornerstone) + D-S08-NN-B LAW (Voice Action Intent Enum). ---
    # Intent 02 voice buy flow internal transient fields (Pattern A reuse).
    # Voice context loaded from Redis key voice:context:{user_id} (separate from
    # RedisSaver checkpoint per D-S08-NN-A; TTL 30min refresh-on-read=True,
    # FIFO 5 turns). Cart snapshot loaded from cart.get MCP (S-05 D-S05-02 LAW
    # JSON shape reuse). Voice action enum: 'add' | 'remove' | 'update_qty'
    # | 'query' per D-S08-NN-B LAW.
    #
    # NOT underscore-prefixed: voice_text + voice_action are surfaced to SSE
    # (voice_transcribed event payload + understanding bubble text). Other 6
    # fields are downstream transient driven by node graph internal state.
    #
    # voice_confidence MAY be None per R-S08-1 (Gemini audio API does not
    ...
    voice_matched_products: list   # Vespa resolve output per item (asyncio.gather)
    voice_clarify_pending: bool    # Pattern A interrupt flag awaiting='clarify_pick'

    # ------------------------------------------------------------------
    # S-10 T01.D (Intent 07 analyzing_by_voices) — ADDITIVE analytics voice
    # fields per D-S10-NN-G LAW (math-first reasoning). total=False so these
    # are optional; only the analytics graph reads/writes them. voice_text /
    # voice_action / voice_history / voice_confidence are REUSED from S-08.
    # ------------------------------------------------------------------
    analyze_metric: str            # classify: revenue|trend|stock|loan|overview
    analyze_dimension: str         # classify: category|product|merchant|month
    analyze_time_range: str        # classify: rolling_7d|week|month
    analyze_filters: dict          # classify: {category?, product?}
    analyze_clarify_pending: bool  # Pattern A interrupt flag awaiting='analyze_clarify'
    _agg_data: dict                # transient: analytics.aggregate result
    _anomaly_data: dict            # transient: analytics.detect_anomaly result
    _stock_data: dict              # transient: analytics.stock_snapshot result
    analyze_reasoning: dict        # build_insights: {price,promo,restock,trend,loan}
    analyze_cards: list            # build_insights: 0..N action cards (numbers+_trace)
    analyze_narrative: str         # narrate: LLM VN prose (NO numbers generated)
