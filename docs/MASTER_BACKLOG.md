...
| 36 | Vespa hybrid_search 400 — rank_profile cross_encoder_rerank reject query | — | 🟡 | rank_profile `cross_encoder_rerank` từ chối query (vd 'nước mắm'+gia_vi); Vespa 68 docs up nhưng rank-profile query bị reject → E_MCP_ERROR → mọi search-intent KHÔNG reach result-state. Phát hiện T02b-hotfix spike. Root cause TBD (rank-profile deploy? query? schema). Chặn T02b-2 search e2e → RECON-first |
