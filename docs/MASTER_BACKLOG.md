...
| S-P0-03 | Safety-net (Cluster C2, ADR-052/054) | P0 | 🟡 | T01 ✅ *(CI soft→hard + coverage ratchet + openapi gate W-46 + deploy-drift wired; pulled 2 web fix + Python ruff/pytest)* · T01b ✅ *(CI integration-db: postgres+redis un-skip 19 spec + mcp stock-atomic; gateway floor 29→43 with-DB; test-vs-RLS fix AC-12)* · T01c-hotfix ✅ *(thread state→identity_kwargs vào _fetch_price_solver; gỡ noqa F821; +2 regression test)* · T02a ✅ *(seed foundation đa-tenant: W-80 seed.ts tenant-aware + fix latent NOT NULL post-V011 · W-75 close ci-min +customer1/tenant#2 a2a2a2a2 · seed-foundation.spec 6 · persist §2 SPLIT-3)* · T02b-1 ✅ *(e2e deterministic: rewrite-flow dashboard+auth-flow AC-38.1 nav onboarding→/s/demo/home sau S-P0-01 · auth+cart 7-route+cross-tenant storefront isolation · storageState login 1×/run né throttle W-60 · 9/9 all-together + leak-demo red→revert→green; e2e CI wire→T02c)* · T02b-hotfix ✅ *(FE 6 stream-hook +X-Tenant-Id; regression S-P0-01 resolver header-only; 400→202 proven unit2/2+e2e; reach-result-state defer→Vespa #36)* · T01d-hotfix ✅ *(baseline CI main đỏ: ci.yml build @icp/shared-types trước typecheck/test/integration-db + coverage SOURCE-only exclude *.spec.ts ADR-057 — sửa nghịch lý thêm-test-tụt-gate T02a; floor re-based gw43→42 honest source-only/web53/workers10)* · T02b-2 ⬜ · T02c ✅ *(e2e deterministic hard-gate MODE B: ci.yml job boot-from-source services pg+redis + W-81 cart contract-Zod + e2e-cleanup direct; 4 spec/17 test, intent-03 EXCLUDED→T02b-2 Vespa#36; gate đỏ-thật proven local)* · T03a ✅ *(V015 llm_traces partition+RLS · housekeeper rolling+3mo · cross-tenant RLS red-when-wrong proof)* · T03b ✅ *(traces.append ADR-003 · llm_client fire-and-forget emit MỌI attempt · V016 status CHECK · trace_id xuyên 3-service live-text · W-54 STALE đã có server-extract; residual: voice-live trace verify→T04 eval live mode, text-live+embedded đã chứng spine modality-agnostic)* · T03c ⬜ · T04 ⬜ · T05 ⬜ |
...
| 36 | Vespa search vỡ tenant-isolation — vespa-feed KHÔNG feed tenant_id (NHÀ S-P0-01) | — | 🟡 | Vespa search vỡ tenant-isolation — vespa-feed.ts KHÔNG feed tenant_id (:163-185, không SELECT) + deploy thuần tay → live Vespa stale pre-T04 (thiếu field) → YQL 400; sau re-deploy field rỗng → match 0. ROOT = T04 làm DỞ feed-side (query-side vespa.py đã filter tenant_id). NHÀ: S-P0-01 (hoàn thiện T04 isolation). Chặn S-P0-03/T02b-2 search(03)+recommend(04) reach-result. Fix = code(feed +tenant_id)+ops(deploy+re-feed); KHÔNG backfill CLI sẵn. (Sửa từ "rank-profile reject" — RECON#36+deep.) |
...
| **C2** | Safety-net (test/CI/eval/golden + obs cost-trace) — TRƯỚC AI-refactor | A13 · W-32,37,**40✅**(llm_client ghi trace MỌI attempt qua MCP traces.append, fire-and-forget+counter; S-P0-03/T03b),**46✅**(openapi gate, S-P0-03/T01),**54✅**(traceparent xuyên Gateway→AI→MCP — inject+server-extract đã có, verified live 3-service cùng trace_id; S-P0-03/T03b),55,56,74,75,**76✅**(soft-fail→hard + coverage ratchet + integration-db un-skip 19 spec + openapi+git_sha gate; S-P0-03/T01+T01b. *residual:* live-smoke compose stage conditional `RUN_DEPLOY_SMOKE=true` cho prod deploy),77,78,79,80,81,**93✅**(V015 llm_traces durable trace/cost spine: partition RANGE monthly + RLS tenant-isolation parent+partition + housekeeper rolling+3mo; S-P0-03/T03a) (**17**) | **S-P0-03** (active) |
