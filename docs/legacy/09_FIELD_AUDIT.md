...
§2 Product Card mapping KHỚP DB 100% — products thật = 22 cột (id,merchant_id,title,description,category,
attributes,price[bigint],stock,image_url,vespa_doc_id,trend_score,status,created_at,updated_at,brand,
original_price,rating_avg,rating_count,sold_count,image_gradient,icon_hint,image_data). KHÔNG có tenant_id (greenfield).
§6/§9 SseProductsEvent + match_score/reason + product_ready CÓ thật (sse/intent-stream.ts).
CHƯA CODE: badge-deriver.ts (§3 — gateway/src/products chỉ có controller+module); sold_count tăng bởi
inventory-consumer (worker skeleton); tenant_id trên products. -->
...
> **Verified DB trực tiếp (2026-06-09):** mọi cột Postgres ở trên TỒN TẠI thật (products = 22 cột). `price`/`original_price` = `bigint`, `rating_avg`/`trend_score` = `real`. **`tenant_id` CHƯA CODE** (products chưa có cột — production multi-tenant ADR-040). `sold_count` cột có thật nhưng cơ chế tăng (worker `inventory-consumer` sau payment) = **CHƯA CODE** (workers skeleton).

## 3. Badge Derivation Rules (LOCKED)

> ⚠️ **CHƯA CODE (verified 2026-06-09):** `apps/gateway/src/products/` thật chỉ có `products.controller.ts` + `products.module.ts` — **chưa có `badge-deriver.ts`**. Logic dưới đây là spec target (badges hiện chưa derive ở BE).
...
→ Cần endpoint `GET /api/v1/dashboard/stats` trả 3 numbers + tránh 3 query riêng. Caching Redis 60s.
...
→ Endpoint `GET /api/v1/dashboard/insight` trả 1 insight `pending` hoặc null. Generated bởi worker-analytics (Phase 05) khi detect anomaly trong analytics_aggregations.
...
| Tile "Phân tích" mini chart | Server: `last_7d_revenue` array | `GET /dashboard/stats` extend |
...
**S-04 Variant B EXCEPTION (Phiên Sx04 per D-S04-03 LAW + AMENDED Phiên Sx04-4 per D-S04-14 LAW):**
Anti-2 applied originally when Intent 03 was BM25-only baseline (no semantic ranking → no defensible match_score). S-04 D-S04-03 LAW introduces Variant B (`mode='ai_augmented'`) which uses Vespa rank profile `ai_augmented` (BM25 + closeness(text_embedding) + trend_score) with `match_score` as a **RANKING-TIME Vespa summary feature** (NOT a Postgres document field — no schema change, no V010 migration). In Variant B mode ONLY, per-product `match_score` (float [0,1]) + `reason` (LLM-generated string max 60 chars) are emitted via SSE `products` event + `product_ready` event (NEW Phiên Sx04-4 per D-S04-14 LAW per-product progressive streaming) — defensible because:
1. `match_score` derived from real semantic similarity (CLIP embedding closeness) NOT vague "AI confidence"
2. `reason` LLM-generated per-query per-product at `generate_reasons` node, grounded in actual product attributes
3. Variant A (`mode='basic_fallback'`) — original Anti-2 rule still applies: NO `match_score` badge, NO `reason` chip (FE conditional render guard per `03_API_CONTRACTS.md §3 products.items[]` augmentation spec)
4. Mockup `intent-03B-state-0-happy.html` line 185 (`98%`) + line 196 ("Độ đậm cao, khách phở hay chọn nhất") = LOCKED UI requirement; mockup `intent-03A-state-0-happy.html` (Variant A) does NOT show these badges/chips

Net: Anti-2 NARROWED to apply to Variant A only + Intent 04 Recommend (original scope); Variant B Intent 03 explicitly allowed per D-S04-03 LAW. See `02_DATA_MODEL.md §2` ai_augmented rank-profile + `03_API_CONTRACTS.md §3` products event augmentation + `slices/S-04_decisions-log.md` C-S04-B resolution.
...
| V004 | — | ⏭️ Skipped | Promotions chưa làm (gap V004) |
| **V005** | P04 | ✅ Done | Payment metadata (failure_reason, payment_method) |
| **V006** | P05 | ✅ Done | Materialized views analytics aggregations |
| V007 | — | ⏭️ Skipped | Original plan: image storage. Superseded by V010 below (cleaner separation: `image_url` stays for future CDN URL; new `image_data` column holds base64 inline). |
| **V008** | P03 | ✅ Done | `V008__shopee_prices_mock.sql` (verified DB) — bảng `shopee_prices_mock` (10 cột) cho enrich Intent 01 (so giá Shopee). ADR-039: production crawler thật → `shopee_prices` (hiện mock). |
| **V009** | P02 | ✅ Done | `V009__auth_refresh_token.sql` (verified DB) — `sessions.refresh_token_hash` + `refresh_expires_at` (refresh-token rotation 30d). |
| **V010** | P03 (S-07 Phiên Sx07-D) | ✅ Done | **S-07 P03 `image_data` inline base64** — ADD COLUMN `products.image_data TEXT NULL` per C-S07-B Option Β (reuse-max: Vespa schema 100% unchanged; S-09 inherits ready column for Option A Gemini-describe-then-embed pipeline). Migration ADD-only; no backfill required. |
| **V011** | P04 (production) | 🟡 CHƯA CODE | Multi-tenant + Payment + GDPR: tenant_id + `payment_callbacks` + status `refunded` + **ALTER `chk_payment_method` thêm `vnpay`** (DB hiện = mock/momo/zalopay/bank_transfer/cod) — `02 §1.X`. |

**Decision: Image storage (amended Phiên Sx07-D per C-S07-B)**
- `products.image_data TEXT` (V010 ADD) holds base64 JPEG inline; `products.image_url VARCHAR(500)` stays for future CDN URL migration post-MVP (semantic separation: data vs URL).
- Mock data ~75 products (verified DB 2026-06-09) × ~50KB ≈ 3.75MB total → Postgres handle fine
- Frontend dùng `<img src="data:image/jpeg;base64,..."/>` không cần CDN
- Vespa schema unchanged (image_url indexed:summary still); image_data stored in PG only.
...
> ⚠️ **Verified routes 2026-06-09 (`dashboard.controller`):** route THẬT = **`GET /api/v1/dashboard/stats`** + **`GET /api/v1/dashboard/insight`** (doc cũ ghi `/me/dashboard-stats` + `/me/insights/latest` = SAI path). Response shape + query SQL dưới = spec (DTO thật `DashboardStatsDto` ở `shared-types/api/models`). Cross-ref `03 §1.7`.
